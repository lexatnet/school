# стоки начинающиеся после символа `#` это коментарии и итнепрпертатор python будет их игнорировать 
...
# каждая корзина для сортировки у нас так же будет представлена листом но пустым так как в них ничего нет по началу
bucket1=[];
bucket2=[];
bucket3=[];
bucket4=[];
...
for i in range(0, len(bucket)):
        ...
        if (
                (((item[color] == 'red') and (item[dimentions] < 2) and (item[weight] > 2)))# * красные, больше 2кг, меньше 2м в диаметре
                or(((item[color] == 'blue') and (item[dimentions] > 2) and (item[weight] < 2))) # * синие, больше 2м в диаметре, меньше 2кг
                or(((item[color] == 'green') and (item[dimentions] < 2) and (item[weight] > 3)))# * зелёные, меньше 2м в диаметре, больше 3кг
        ): # условный оператор поможет нам проверить условия
                bucket1.append(item)
                continue
        if (
                ...
                bucket2.append(item)
                continue
        if ((((item[color] == 'blue') and (item[dimentions] > 2) and (item[dimentions] < 4)))): #* синие, больше 2м но меньше 4х в диаметре.
                bucket3.append(item)
                continue
        bucket4.append(item)
...
bucketInfo('bucket 1', bucket1)
bucketInfo('bucket 2', bucket2)
bucketInfo('bucket 3', bucket3)
bucketInfo('trash', bucket4)
