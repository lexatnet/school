...
for i in range(0, len(bucket)):
	item = bucket[i]; # берём очередной шар под номером `i` из корзины и помещаем его в переменную `item`
	...
	itemOutput(bucket[i]) # можно было написать так `itemOutput(item)`
